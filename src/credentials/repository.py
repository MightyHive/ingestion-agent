"""This module executes SQL for credentials
connections. Every lookup is constrained by tenant_id to enforce isolation.
"""
...
class ConnectionRepository:
    """CRUD operations for ``connections`` records.

    Parameters
    ----------
    session:
        Active SQLAlchemy session controlled by the caller.
    """
    ...
    def create(self, data: ConnectionCreate) -> ConnectionRecord:
        """Create a new connection metadata row.

        Tokens are intentionally excluded from this contract; ``secret_id`` stores
        only a reference to Secret Manager.
        """
    ...
    def create_with_connection_id(
        ...
        """Create a row with a caller-provided stable connection id."""
    ...
    def get(self, tenant_id: str, connection_id: str) -> ConnectionRecord | None:
        """Fetch one connection by tenant + id."""
    ...
    def list_by_tenant(
        ...
        *,
        ...
        """List connections that belong to a tenant."""
    ...
    def update_status(
        ...
        """Update status for one tenant-scoped connection."""
    ...
    def update_metadata(
        ...
        *,
        ...
        """Update mutable metadata fields for one connection."""
    ...
    def _require_connection(self, *, tenant_id: str, connection_id: str) -> ConnectionRow:
        """Return row or raise a tenant-scoped not-found error."""

        stmt = select(ConnectionRow).where(
            # tenant_id in WHERE is mandatory to prevent cross-tenant access.
            ConnectionRow.tenant_id == tenant_id,
            ConnectionRow.connection_id == connection_id,
        )
        row = self._session.scalar(stmt)
    ...
    def _to_record(row: ConnectionRow) -> ConnectionRecord:
        """Map ORM row to Pydantic record."""
